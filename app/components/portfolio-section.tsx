  ...
  return (
    ...
    >
      <div className="flex flex-col w-full max-w-[722px] h-[375px] gap-10 lg:justify-center px-[17px] py-[76px] md:py-[92px] md:px-0 xl:px-[62px] xl:max-w-[1440px]">
        {/* container section */}
        <div
          className={`flex flex-row text-[#B3B3B3] font-normal md:text-3xl text-2xl py-2 ${leckerliOne.className}`}
        >
          Portfolio...
        </div>
    ...
    <div className="relative fit  overflow-hidden group cursor-pointer">
      ...
      <div className="absolute w-full h-full inset-0 flex items-center justify-center opacity-0 bg-gray-800 bg-opacity-75 transition-opacity duration-500 group-hover:opacity-100">
        <div className="flex flex-col justify-end w-full ml-4 text-white h-full py-5">
          <h5
            className={`mx-2 sm:text-sm lg:text-xl md:text-lg  font-normal ${leckerliOne.className} `}
          >
            {title}
          </h5>
          <h2 className=" flex justify-start font-extrabold  mx-2 lg:text-4xl sm:text-xl md:text-lg ">
            {description}
          </h2>
        </div>
      </div>
    ...
    <div className="flex flex-col w-full">
      ...
      <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-1">
        {projectImages.map(({ img, title, description }) => (
          <ImageCardInfo key={title} {...{ img, title, description }} />
        ))}
      </div>
    </div>
  );
